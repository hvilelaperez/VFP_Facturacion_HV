...
SELECT ALLTRIM(codtipart) + ' '+ destipart AS Tipo_Art, TIPO, TD, IIF(TD= 'SI', 00, MONTH(Fecha)) as Mes, SUM(CANTIDAD) AS CANTIDAD, SUM(CT_SOL) AS COSTO_TOT FROM CKARDEX GROUP BY Tipo_Art, TIPO, TD,Mes
COPY TO c:\sistemas\temporal\kardex_cprom TYPE xls
...
cmd1 = SQLEXEC(lnHandle,"Select 'I' as Tipo, Case When TipoOrigen= 'D' Then 'VD' Else 'VI' End as TD, id as unico, fingreso as fecha, '    ' as codigoc, id, producto, qing as cantidad, um, 00.00 as precio, id as id_vtaslote, "+ ; 
	"costo as CU_Dol, 0000.0000 as CU_Sol, 00000000.00 as CT_Sol, 00000000.00 as Sld_Can, 00000000.00 as Sld_Sol, 00.000 as TC "+ ;
	"From vtalotes Where fingreso >= ?MyCierre ", "cIngresos")

WAIT WINDOW 'Preparando el kardex ... 2 ' NoWait

cmd2 = SQLEXEC(lnHandle,"Select 'S' as Tipo, 'FB' as TD, fc.unico, fc.fecha, fc.codigoc, fd.id, fd.producto, fd.cantidad, fd.um, fd.precio, fd.idorigen as id_vtaslote "+;
	", 0000.0000 as CU_Dol, 0000.0000 as CU_Sol, 00000000.00 as CT_Sol, 00000000.00 as Sld_Can, 00000000.00 as Sld_Sol, 00.000 as TC "+ ;
	...
	"union all "+ ;
	"Select 'S' as Tipo, 'FB' as TD, bc.unico, bc.fecha, bc.codigoc, bd.id, bd.producto, bd.cantidad, bd.um, bd.precio, bd.idorigen as id_vtaslote "+ ;
	", 0000.0000 as CU_Dol, 0000.0000 as CU_Sol, 00000000.00 as CT_Sol, 00000000.00 as Sld_Can, 00000000.00 as Sld_Sol, 00.000 as TC "+ ;
	...
	"union all "+ ;
	"Select 'S' as Tipo, 'VS' as TD, ec.unico, ec.fecha, '    ' as codigoc, ed.id, ed.producto, ed.cantidad, ed.um, 0.00 as precio, ed.unicoorigen as id_vtaslote "+ ;
	", 0000.0000 as CU_Dol, 0000.0000 as CU_Sol, 00000000.00 as CT_Sol, 00000000.00 as Sld_Can, 00000000.00 as Sld_Sol, 00.000 as TC "+ ;
...
SELECT 'I' as Tipo, 'SI' as TD, 0000 as unico, fingreso as fecha, '    ' as codigoc, 0 as id, codigo as producto, stock as Cantidad, '  ' as UM, 0.00 as precio, 000000 as id_vtaslote, ;
 CU_Dol, 0000.0000 as CU_Sol, 00000000.00 as CT_Sol, 00000000.00 as Sld_Can, 00000000.00 as Sld_Sol, 00.000 as TC, LEFT(codigo,2) as CodTipArt, SPACE(40) as DesTipArt FROM cSaldoIni ;
Union all ;
