...
LOCAL HayAnticipos,TotalAnticipos &&&&DIFANTICIPOS
...
mPrueba = 2    && variable temporal para realizar las pruebas

MitipoDoc='F' &&x_Sunatipodoc
Seriedoc=10 &&Myserief
IF mPrueba = 1
	Numerodoc=3972 &&Mynumerof    && Prueba 1
ELSE
	Numerodoc=3977 &&Mynumerof    && Prueba 2
ENDIF
...
SELECT MisAnticipos &&DIFANTICIPOS
IF RECCOUNT()>0     &&DIFANTICIPOS
	HayAnticipos=1   &&DIFANTICIPOS
	SUM ALL MisAnticipos.monto TO TotalAnticipos  &&DIFANTICIPOS
ELSE                &&DIFANTICIPOS
	HayAnticipos=0   &&DIFANTICIPOS
	TotalAnticipos=0 &&DIFANTICIPOS
ENDIF               &&DIFANTICIPOS
...
FraseDetrac03=ALLTRIM(STRCONV(KonsMess.c_frasedetrac03,11))
...
IF mPrueba = 2
	b_Parte01=MitipoDoc+TRANSFORM(serie,'@L 999')+'-'+TRANSFORM(3978,'@L 99999999')
ENDIF
...
DO CASE
	CASE MitipoDoc='F'
		IF SunatMidoc.directa=1
			i_Parte01=STRCONV(ALLTRIM(SunatMidoc.ocdirecta),11)  && Ver si es asi NO va OC
		ELSE
			IF SunatMidoc.agrupada=1
			...
			ELSE
				...
				i_Parte01=ALLTRIM(STRCONV(LocMx.oc,11)) && Orden de Compra  (Busqueda en Guia de Remision)
			ENDIF
		ENDIF
	CASE MitipoDoc='B'
		i_Parte01=''
...
********Inicio Parte04: Totales ****************************************************************************************************************************
***&&& Sub para IGV
a_Parte04=ALLTRIM(STR(SunatMidoc.igv,12,2))  && Sumatoria IGV
*** Borrar la siguiente linea despues de prueba:
IF mPrueba = 1
	a_Parte04=ALLTRIM(STR(0,12,2))  && Sumatoria IGV. Valor 0, para el caso de despacho parcial.
ELSE
	a_Parte04=ALLTRIM(STR(7.29,12,2))  && Sumatoria IGV. Valor 0, para el caso de despacho parcial.
EndIf
***
...
ELSE
	...
	SCAN
		y_PyearAnRe=STR(YEAR(f_recepcionpago),4,0)
		y_PmontAnRe=TRANSFORM(MONTH(f_recepcionpago),"@L 99")
		y_PDayyAnRe=TRANSFORM(DAY(f_recepcionpago),"@L 99")
		fectxtAnRe=y_PyearAnRe+'-'+y_PmontAnRe+'-'+y_PDayyAnRe

		y_PyearAnEf=STR(YEAR(f_efectuadapago),4,0)
		y_PmontAnEf=TRANSFORM(MONTH(f_efectuadapago),"@L 99")
		y_PDayyAnEf=TRANSFORM(DAY(f_efectuadapago),"@L 99")
		fectxtAnEf=y_PyearAnEf+'-'+y_PmontAnEf+'-'+y_PDayyAnEf

		a_Parte06=ALLTRIM(STR(monto,12,2))
		b_Parte06=ALLTRIM(tipo)
		c_Parte06=ALLTRIM(numfactura)
		d_Parte06=fectxtAnRe
		e_Parte06=fectxtAnEf
		f_Parte06='12:00:00'
		
		x06_StrBodySub=x06_StrBodySub+a_Parte06+'|'+b_Parte06+'|'+c_Parte06+'|'+d_Parte06+'|'+e_Parte06+'|'+f_Parte06+'|'+'}'
	ENDSCAN
	x06_StrBody=x06_StrBodySub+'~'
ENDIF
********Fin Parte06 ****************************************************************************************************************************************

********Inicio Parte07: Detalle ****************************************************************************************************************************
...
IF Isvtasucesiva=0
	...
	SCAN
		...
		var7=subtotal
		var8=es_alter
		var9=STRCONV(txt_alternativo,11)
		SELECT Sunatmidetalle
...
SCAN
	...
	IF MitipoDoc='F' AND Miserie=60  &&& Es Vta Sucesiva
		IF CtaMiReg=CtaRegVs  && Es el ultimo registro del detalle
			a_Parte07=ALLTRIM(STRCONV(Sunatmidetalle.nombre,11)) &&&+'^^^'+'^ORIGEN:'+ALLTRIM(STRCONV(Datavs.paisorigen,11))+'^'+ALLTRIM(STRCONV(Datavs.aranceles,11))+'^'+NotaVs01
			b_Parte07='^^'+'^ORIGEN:'+ALLTRIM(STRCONV(Datavs.paisorigen,11))+'^'+ALLTRIM(STRCONV(Datavs.aranceles,11))+'^'+NotaVs01 &&Detalle adicional
		ELSE
			a_Parte07=ALLTRIM(STRCONV(Sunatmidetalle.nombre,11))
			b_Parte07=''  && Detalle adicional
		ENDIF
	ELSE
		IF Sunatmidetalle.es_alter=0
			a_Parte07=ALLTRIM(STRCONV(Sunatmidetalle.nombre,11))+' '+ALLTRIM(STRCONV(Sunatmidetalle.lotefab,11))
			b_Parte07=''  && Detalle adicional
		ELSE
			a_Parte07=ALLTRIM(STRCONV(Sunatmidetalle.txt_alternativo,11))
			b_Parte07=''  && Detalle adicional
		ENDIF
	ENDIF
	c_Parte07=ALLTRIM(STRCONV(Sunatmidetalle.producto,11))
	IF ALLTRIM(UPPER(Sunatmidetalle.um))='UN'
		d_Parte07='NIU'  && NIU
	ELSE
		d_Parte07=UPPER(ALLTRIM(STRCONV(Sunatmidetalle.um,11))) && Catalogo 03
	ENDIF
	...
	IF MitipoDoc='F' AND Miserie=60  && Si es Factura Vta Sucesiva
		w_Parte07= ALLTRIM(STRCONV(Sunatmidetalle.lotefab,11))
	ELSE
...
*** Borrar la siguiente linea despues de prueba:
IF mPrueba = 1
	b_Parte05=ALLTRIM(STR(0,12,2)) && Total de la Factura o Boleta
ELSE
	b_Parte05=ALLTRIM(STR(47.79,12,2)) && Total de la Factura o Boleta
ENDIF
***

c_Parte05='' &&ALLTRIM(STR(0.00,12,2)) && Descuentos (NO manejamos)
d_Parte05=ALLTRIM(STR(TotalAnticipos,12,2))  &&DIFANTICIPOS

x05_StrBody=a_Parte05+'|'+b_Parte05+'|'+c_Parte05+'|'+d_Parte05+'|'+'}'+'~'  &&DIFANTICIPOS

********Fin Parte05 ****************************************************************************************************************************************

********Inicio Parte08: Totales por opereración ************************************************************************************************************
		...
		CASE Miserie=10 OR Miserie=20 OR Miserie=40  OR Miserie=30 OR Miserie=50 OR Miserie=60 OR Miserie=70 OR Miserie=80 && Revisar si es importacion
			DO CASE
				...
				OTHERWISE
					...
					*** Borrar la siguiente linea despues de prueba:
					IF mPrueba = 1
						a_Parte08=ALLTRIM(STR(0,12,2))  && Gravada
					ELSE
						a_Parte08=ALLTRIM(STR(40.50,12,2))  && Gravada
					ENDIF
					***
...
IF MitipoDoc='F' AND Miserie=60 &&& Solo Para Vta Sucesiva
	a_Parte09=ALLTRIM(STRCONV(Datavs.bl,11))  && BL Conocimiento de Embarque
	e_Parte09=ALLTRIM(STRCONV(Datavs.incoterm,11))  && Descripcion INCOTERMS
	...
	k_Parte09=ALLTRIM(STRCONV(Datavs.pedido,11))  && Lista de Empaque / OC
ELSE
...
DO CASE
	CASE MitipoDoc='B'
		...
		ELSE
			b_Parte09=ALLTRIM(STRCONV(SunatMidoc.Nompago,11))
		ENDIF
	OTHERWISE && Factura
		IF Miserie=60  &&& Si es Vta Sucesiva
			b_Parte09=ALLTRIM(STRCONV(Datavs.condicionpago,11))
		ELSE
			b_Parte09=ALLTRIM(STRCONV(SunatMidoc.Nompago,11))
		ENDIF
ENDCASE
...
DO CASE
	CASE MitipoDoc='B' && Boleta
		IF ALLTRIM(SunatMidoc.condipag)='MG'
			d_Parte09=FraseMG+' '+IIF(d_Parte01='PEN','NS/.','US$')+' '+ALLTRIM(STR(AcumulaMG,12,2))+' '+FraseBoleta
		ELSE
...
l_parte09=alltrim(Convnum(VAL(b_Parte05)))+iif(d_Parte01='PEN'," NUEVOS SOLES", " DOLARES AMERICANOS")   && Letras del Monto total
...
m_parte09='egutierrez@sociedadquimica.com.pe' &&MylistaCorreo  &&  && && &&MylistaCorreo  && &&  &&&  &&   &&MylistaCorreo &&'wtone@sociedadquimica.com.pe;cservat@sociedadquimica.com.pe'  &&& Mail receptor MylistaCorreo
