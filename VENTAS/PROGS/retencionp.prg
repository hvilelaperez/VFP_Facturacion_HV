...
IF lnHandle > 0
	...
	**cmd4=SQLEXEC(lnHandle,"SELECT venta FROM tcoficial WHERE fecha=?hoy","TC")
...
c_Parte02 = ALLTRIM(STRCONV(Empresa.nombre,11))
d_Parte02 = ALLTRIM(STRCONV(Empresa.nombre,11))
e_Parte02 = ALLTRIM(STRCONV(SunatDE.c_myubigeo,11))
f_Parte02 = ALLTRIM(STRCONV(Empresa.direccion,11))
g_Parte02 = ALLTRIM(STRCONV(SunatDE.c_myurbanizacion,11))   
h_Parte02 = ALLTRIM(STRCONV(Empresa.prov,11))
i_parte02 = ALLTRIM(STRCONV(Empresa.dpto,11))
j_parte02 = ALLTRIM(STRCONV(Empresa.dist,11))
k_Parte02 = ALLTRIM(STRCONV(Empresa.pais,11))  && Basado en Catalogo 04
...
DO WHILE !EOF()
	...
	** Datos del proveedor
	...
	IF ATC(ALLTRIM(m.dist), m.direccion) > 0   && Si el distrito está dentro de la dirección, no enviarlos por separado.
		...
		j_parte03 = ''
	ELSE
		h_Parte03 = ALLTRIM(STRCONV(m.prov,11))
		i_parte03 = ALLTRIM(STRCONV(m.dpto,11))
		j_parte03 = ALLTRIM(STRCONV(m.dist,11))
	ENDIF
	
	k_Parte03 = ALLTRIM(STRCONV(m.pais,11))  && Basado en Catalogo 04
